...
#include <iostream>
// создание упорядоченной таблицы по просматриваемой
...
PTDatValue TSortTable::FindRecord(TKey k) {
    int left = 0, right = DataCount - 1, i;
    while (left <= right)
    {
        ++Efficiency;
        i = (left + right) / 2;
...
void TSortTable::QuickSort(PTTabRecord *pRecs, int DataCount) {
    ...
    if (DataCount > 1)
    {
        QuickSplit(pRecs, DataCount, piv);
        QuickSort(pRecs, piv);
        QuickSort(pRecs + piv, DataCount - piv);
    }
}
//быстрая соритровка - выбор ведущего элемента и разделение данных
